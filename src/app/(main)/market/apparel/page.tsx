...
export default async function ApparelPage() {
  ...
  const items = await getListingsByCategory("apparel");

  return (
    <div className="mx-auto max-w-6xl px-4 py-6">
      <PageHeader title={t("apparel.title")} description={t("apparel.description")} />
      <div className="mt-5 flex items-center gap-2 rounded-xl border border-info/30 bg-info/10 px-4 py-3">
        ...
        <span className="text-xs text-fg-muted">{t("apparel.vendorNotice")}</span>
      </div>
      <p className="mt-5 text-xs text-fg-subtle">
        {t("apparel.resultCount", { count: items.length })}
      </p>
      <div className="mt-3">
        <ListingGrid>
          {items.map((l) => (
